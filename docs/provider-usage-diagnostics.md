...
The per-turn `provider_usage` diagnostic and PR16 context attribution paths are
disabled by default. Enable both explicitly for an AionCore process with:
...
value, including an unset variable, leaves them disabled. The toggle does not
disable the existing UI usage indicator, lifecycle records, or the separate
`context_generation` and `provider_correlation` metadata diagnostics.
